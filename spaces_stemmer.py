...
class stemmer:
    ...
    """
    vc_count = re.compile(r'[y]?[^aeiouy]*[aeiouy][aeiou]*[^aeiou][^aeiouy]*').findall
    ...
    def m(self, word):
        ...
        VC measure.
        """
        return len(stemmer.vc_count(word))
    ...
    def longest_match (self, word, tbl, m_thresh = 1):
    ...
    def replace_ends_if(self, suffix_tbl, cond = True, m_thresh = 1):
        ...
        if not cond:
            return self.stemmed
        longest_match = self.longest_match (self.stemmed, suffix_tbl, m_thresh)
        if longest_match[0] and self.stem_m(longest_match[0]) > m_thresh:
            self.stemmed = self.rep_end(self.stemmed, longest_match[0], longest_match[1])
        return self.stemmed
    
    def v_in_stem(self, word, suffix = '', rep = ''):
        """Whether or not word conatins a vowel."""
        return stemmer.contains_v(self.rep_end(word, suffix, rep))
    
    def stem_ends_with_dbl_cons(self):
        """Whether or not the stem ends with double consonant. """
        return stemmer.double_c(self.stemmed)
    ...
    def stem_ends_with_cvc(self, suffix = '', rep = ''):
        ...
        return stemmer.ends_cvc(new_stem)
    
    def str_ends_with_char(self, word, char_list = []):
    ...
    def step1ab(self):
        ...
        if self.stemmed.endswith('eed'):
            ...
            self.step1b_third_rule = self.replace_end('ing', '', self.v_in_stem(self.stemmed, 'ing'))
    
    def step1b1(self):
        """The rule to map to a single letter causes the removal of one of the double
        letter pair. This rule puts the -E back on -AT, -BL and -IZ, so that the suffixes
        -ATE, -BLE and -IZE can be recognised later. This E may be removed in step
        4. (This rule is applied only if the second or third rule of step1b was satisfied.)
        """
        ...
        self.remove_dbl_last_char(self.stem_ends_with_dbl_cons() 
                    and not self.str_ends_with_char(self.stemmed, ['l', 's', 'z']))
        
    def step1c(self):
        """Replaces 'y' by 'i' if the stem will have a vowel after y is removed."""
    ...
    def step4(self):
        ...
        longest_match = self.longest_match (self.stemmed, self.step4_suffix_tbl, 1)
        if self.stemmed.endswith('ion') and len(longest_match[0]) < len('ion'):
            if self.str_ends_with_char(self.rep_end(self.stemmed, 'ion'), ['s', 't']):
                longest_match = ('ion', '')
        if self.stem_m(longest_match[0]) > 1:
            self.stemmed = self.rep_end(self.stemmed, longest_match[0], longest_match[1])
            
    def step5(self):
        """step5() removes a last -e if stem measure > 1, and changes -ll to -l if
            stem measure > 1.
            """
    ...
    if len(sys.argv) > 1:
        ...
        for f in sys.argv[1:]:
            for line in file(f, 'r'):
                output += [s.stem(word) for word in line.split()]
        print "\n".join(output)
